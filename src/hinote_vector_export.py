...
def render_pdf_page(pdf: bytes, page_index: int, target_width: float, target_height: float) -> tuple | None:
    """Render a PDF page as a JPEG image using PyMuPDF (fitz).

    Preserves the PDF's native aspect ratio (``min`` zoom, "contain" mode)
    and returns the JPEG bytes together with the centred placement rect
    within the target page, so the caller can letterbox the image.

    Returns ``(jpeg_bytes, x, y, width, height)`` or ``None`` on failure.
    """
    try:
        import fitz
    except ImportError:
        print("warning: PyMuPDF (fitz) 未安装，无法渲染 PDF 背景。运行 `pip install pymupdf` 后重试。")
        return None
    try:
        doc = fitz.open(stream=pdf, filetype="pdf")
    except Exception as exc:
        print(f"warning: 无法打开 PDF 流：{exc}")
        return None
    ...
    # 保持 PDF 原始宽高比，等比例缩放至完全适配目标尺寸（contain 模式）
    scale = min(target_width / pw, target_height / ph)
    rendered_w = pw * scale
    rendered_h = ph * scale
    ...
    # 返回居中放置的位置参数
    return (jpeg_bytes,
            (target_width - rendered_w) / 2,
            (target_height - rendered_h) / 2,
            rendered_w,
            rendered_h)
...
def build_page(page_data: dict, files: dict[str, bytes], bkg_map: dict[str, str] | None = None) -> Page:
    ...
    # 背景：PDF 导入页（bkgAttachmentId 指向 *_pdf）或图像导入页（指向 *_image）。
    # 解析为满页 JPEG，作为最底层 ImageElement 插入 images 首位（先绘制=底层）。
    bkg_id = page_data.get("bkgAttachmentId") or ""
    if bkg_id and bkg_map and bkg_id in bkg_map:
        bkg_name = bkg_map[bkg_id]
        bkg_data = files.get(bkg_name)
        if bkg_data:
            bkg_bytes = None
            bkg_mime = None
            if bkg_name.endswith("_pdf"):
                # PDF 背景：用 PyMuPDF 等比例渲染指定页面为 JPEG，居中放置
                idx = page_data.get("bkgAttachmentIndex", 0) or 0
                result = render_pdf_page(bkg_data, idx, width, height)
                if result is not None:
                    bkg_bytes, bkg_x, bkg_y, bkg_w, bkg_h = result
                    bkg_mime = "image/jpeg"
                    images.append(ImageElement(
                        data=bkg_bytes, mime_type=bkg_mime,
                        x=bkg_x, y=bkg_y, width=bkg_w, height=bkg_h, angle=0.0,
                    ))
                    bkg_bytes = None  # 防止下方 if bkg_bytes 重复添加
            elif bkg_name.endswith("_image"):
                if bkg_data.startswith(b"\xff\xd8\xff"):
                    bkg_bytes = bkg_data
                    bkg_mime = "image/jpeg"
                elif bkg_data.startswith(b"\x89PNG\r\n\x1a\n"):
                    bkg_bytes = bkg_data
                    bkg_mime = "image/png"
            if bkg_bytes:
                images.append(ImageElement(
                    data=bkg_bytes, mime_type=bkg_mime,
                    x=0.0, y=0.0, width=width, height=height, angle=0.0,
                ))
    texts: list[TextElement] = []
...
def export_archive(archive_path: Path, output_root: Path) -> Path:
    ...
    with zipfile.ZipFile(archive_path) as archive:
        ...
        page_data: list[dict] = []
        # 笔记级附件映射：bkgAttachmentId -> 背景文件名（*_pdf / *_image）
        bkg_map: dict[str, str] = {}
        for info in archive.infolist():
            ...
            elif is_penkit_block(data):
                unsupported.append(f"{name}: PENKITINFENG infinite-canvas block (see hinote_infinite.py)")
            elif "/" not in info.filename and name.endswith(".jhinote") and name != "custom_md.jhinote":
                # 顶层 {noteId}.jhinote：解析 attachment[] 建立 id -> filePath 映射
                try:
                    note_obj = decode_jhinote(data)
                    for att in note_obj.get("customNoteContent", {}).get("attachment", []):
                        fp = att.get("filePath", "")
                        if fp:
                            bkg_map[att.get("id", "")] = source_name(fp)
                except Exception:
                    pass

        for data in sorted(page_data, key=lambda item: item.get("pageNumber", 0)):
            page = build_page(data, files, bkg_map)
