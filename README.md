...
`PENCILENGINE` 笔迹会导出为 SVG `<path>`，并汇总为一个 PDF；两种输出都保持笔迹为矢量路径，缩放不会失真。导出严格按原始的“样式记录、点表、索引记录”笔画链解析，读取逐点压感、画布单位的笔刷宽度、颜色和荧光笔透明度，并保留页面背景与 JPEG/PNG 元素。每个原始笔画会生成一条连续的可变宽度填充轮廓，避免逐段圆帽造成的凸起和断裂。输出在 `vector-export/<文件名>/`。
